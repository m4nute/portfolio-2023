...
function Button({ label }: { label: string }) {
    ...
    return (
        <Tooltip
            ...
            offset={12}
        >
            <ActionIcon radius='md' color={useTheme('light.2', 'light.0')} sx={{ height: '2.3rem', width: '2.3rem', transition: '0.2s all ease', ":hover": { scale: '1.1', backgroundColor: useTheme('#505050', '#CBCBCB') } }} variant='filled' className="bg-navbtn rounded-xl p-1 hover:scale-110 transition-transform duration-200">
                {icon}
            </ActionIcon>
        </Tooltip>
    );
