...
function Links({ repo, web }: { repo: string | undefined, web: string | undefined }) {
    ...
    return (
        <Flex justify='center' sx={{ paddingTop: '1.5em', '@media (max-width: 700px)': { paddingBottom: '1.5em' } }} gap="md">
            ...
            {web && <Button size='sm' variant='subtle' color='dark' sx={{ transition: '.2s all ease', display: 'flex', ":hover": { backgroundColor: useTheme('#212121', '#f6f6f6'), scale: '1.05' } }} onClick={() => window.open(web)}> <Box sx={{ paddingRight: '.3em' }}>Website</Box> <IconAppWindow size={20} /></Button>}
        </Flex>
    )
}
